...
지원 분야는 SK하이닉스 **SYSTEM SW**(2027학년도 전기 고려대 반도체시스템공학과
모집분야 표기)이며, AI 메모리/스토리지 시스템 소프트웨어를 중심축, SSD System
Architecture / FTL을 보완 축으로 둔다. 이 문서의 두 프로젝트는
...
기각 자체보다 중요한 것은 그 뒤에 남은 관찰이다. **실제로 시뮬레이션한 조건은 weight
FP16·batch 16**이며, 그 측정된 DRAM traffic 분해는 weight **81.2 %** / KV payload
**12.5 %** / KV metadata **6.3 %**였다(측정 바이트: 436,207,616 / 67,117,056 / 33,622,016).

여기에 두 scaling law(weight는 batch 불변·bit-width 선형, KV는 batch 선형)를 적용하면
**INT4·batch 64** 조건이 다음과 같이 **예측**된다.

| 조건 | weight | KV payload | KV metadata |
|---|---:|---:|---:|
| FP16·batch 16 — **측정** | 81.2 % | 12.5 % | 6.3 % |
| INT4·batch 64 — **분석적 projection** | 21.3 % | 52.4 % | **26.3 %** |

즉 저비트·대배치로 갈수록 **양자화 metadata가 weight보다 많은 바이트를 옮길 수 있다**는
예측이 나온다. 양자화를 더 공격적으로, 더 세밀한 그룹으로 밀수록 이 항은 줄지 않고 커지기
때문이다. payload를 작게 만들기 위한 장부가 다음 병목이 될 수 있다 — 원래 가설이 던지지도
않았던 질문이다.

**다만 INT4·batch 64 조건 자체는 시뮬레이션하지 않았다.** 위 표의 두 번째 행은 측정된
기준점에서 산술로 확장한 projection이며, 실측 결과가 아니라 후속 시뮬레이션으로 검증할
연구 가설이다.

측정이 강제한 두 번째 정정: "전환은 드물다(그래서 공유 엔진이 K/V를 몰아
